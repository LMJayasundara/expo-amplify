...
export default function VerifyResetScreen() {
  const { verifyResetCode, resendCode, isLoading } = useContext(AuthContext);
  ...
  const handleResend = async () => {
    try {
      await resendCode('stored-email', 'reset');
    } catch (err) {
    ...
    >
      ...
      >
        ...
        <View className="flex-1 justify-center p-6 bg-white">
          <Text className="text-2xl font-bold mb-6 text-center">Verify Reset Code</Text>
